...
C++ 代码到可执行程序过程：
```bash
# 1. 预处理（preprocessing）：生成 main.ii，会把引入的包等都拷贝到一个文件中，还是源代码。
# 有了这个源码就不需要头文件了。生成的文件比较大。
# 一些宏定义错误会暴露出来
g++ -E main.cpp > main.ii
# 或者使用下面的工具: cpp is The C Preprocessor
cpp main.cpp

# 2. 编译（compilation）：生成汇编 main.s，里面是汇编代码。会把语法错误暴露出来，针对单个CPP文件，所以关联性错误它不知道。
# 函数声明有了就不会报错，函数定义问题会在链接时暴露
g++ -S main.ii

# 3. 汇编（assembly）：生成二进制 main.o，机器码文件。
g++ -c main.s
...
|-O[0, 1, 2, 3]|对生成的代码进行优化，中括号中的部分为优化级别，缺省的情况下为2级优化，0为不进行优化。注意，采用更高级的优化并不一定得到效率更高的代码|
|-Idir|在编译源文件时增加一个搜索头文件的额外目录dir，即include增加一个搜索的额外目录|
|-Ldir|在编译源文件时增加一个搜索库文件的额外目录dir|
|-llibrary|在编译链接文件时增加一个额外的库，库名为liblibrary.so|
|-w|禁止所有警告。一般代码发布时不显示警告，调试期间打开警告|
|-Wwarning|运行产生warning类型的警告，warning可以是 main, unused等很多取值，最常用是-Wall，表示产生所有警告。如果warning取值为error，含义是将所有警告作为错误（error），即出现警告就停止编译。|
